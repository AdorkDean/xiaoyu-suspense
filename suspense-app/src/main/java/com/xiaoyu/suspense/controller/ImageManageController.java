...
public class ImageManageController extends BaseController {
	...
	//private static final String DEFAULT_FUN_IMAGE = "suspense";
	
	//private static final String DEFAULT_S_FUN_IMAGE = "s_suspense";
     ...
     * @param response
     */
    @RequestMapping(value="/suspense/get/original/{picName}", method=RequestMethod.GET)
    public void getFunPicOriginal(@PathVariable String picName, HttpServletResponse response){
    	String folder = FileConstants.POST_PICTURE  + File.separator;
    	//Random rand = new Random();
     ...
     * @param response
     */
    @RequestMapping(value="/suspense/get/thumbnail/{picName}", method=RequestMethod.GET)
   ...
   * @param response
   */
    @RequestMapping(value = "/suspense/save", method = RequestMethod.POST)
        ...
        try {
            if(request instanceof MultipartHttpServletRequest){
                ...
                if(null!=file){
                    ...
                    if(file.getSize()>100000) { //大于100多KB
	                    //获得上传图片的宽度
	                    int width = getPicWidth(file);
	                    if(width != -1 && width != 0 && width > 200){
	                    	double times = width / 200.0;
	                    	int height = (int)(getPicHeight(file) / times);
	                    	changeImage(file, 200, height, out_thumb);
	                    }
                    }
                    else {                   
                    	changeImage(file, ImageIO.read(file.getInputStream()).getWidth(), ImageIO.read(file.getInputStream()).getHeight(), out_thumb);
                    }
