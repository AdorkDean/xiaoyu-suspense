...
import com.xiaoyu.suspense.entity.BusiAnswerquestion;
import com.xiaoyu.suspense.entity.BusiReport;
import com.xiaoyu.suspense.entity.BusiUser;
import com.xiaoyu.suspense.service.BusiAnswerquestionService;
...
import com.xiaoyu.suspense.vo.BusiUserVO;
...
//<=================定制内容开始==============
	...
	public void login(String bsusrCode, String bsusrPwd, String version, HttpServletRequest request, HttpServletResponse response) {
		...
		Integer collectCount = busiCollectService.getCountBusiCollectByMy(model.getBsusrUuid());
		Integer postCount = busiPostService.getCountBusiPostByMy(model.getBsusrUuid());
		Integer quizCount = busiQuizService.getCountBusiQuizByMy(model.getBsusrUuid());
		vo.setCollectCount(collectCount==null?0:collectCount);
		vo.setPostCount(postCount==null?0:postCount);
		vo.setQuizCount(quizCount==null?0:quizCount);
		//super.removeSession(request);
		//super.setSession(request, bsusrCode);
	...
	public void findUesrInfo(String bsusrUuid, String attentUserUuid, HttpServletResponse response) {
		...
		BusiUserVO vo = new BusiUserVO();
		vo.convertPOToVO(busiUser);
		Integer collectCount = busiCollectService.getCountBusiCollectByMy(bsusrUuid);
		Integer postCount = busiPostService.getCountBusiPostByMy(bsusrUuid);
		Integer quizCount = busiQuizService.getCountBusiQuizByMy(bsusrUuid);
		vo.setCollectCount(collectCount==null?0:collectCount);
		vo.setPostCount(postCount==null?0:postCount);
		vo.setQuizCount(quizCount==null?0:quizCount);
		//判断是否关注过
		BusiReport busiReport = new BusiReport();
		busiReport.setBsrptAttention(attentUserUuid);
		busiReport.setBsrptBeAttention(bsusrUuid);
